...
import getContactUs from "@/app/libs/getContactUs"
import Link from "next/link"

const HomeMap = dynamic(() => import("@/app/_components/HomeMap"))

export default async function ContactUs() {
  const contactUsData = await getContactUs()
  const instagramLink = contactUsData.data.info_tol[0].link
  const facebookLink = contactUsData.data.info_tol[1].link
  ...
  return (
    <div className="text-[#3A354F] bg-[#f7f7f8] w-full py-6 px-4 sm:px-10">
      ...
      <div className="bg-white rounded-md shadow-md px-6 py-10 grid md:grid-cols-2 grid-cols-1 gap-3">
        <div>
          <UnsafeComponent html={contactUsData.data.alamat_1.alamat} /> <br />
          <div className="flex">
            <Link href={instagramLink} target="_blank" className="bg-[#3b5998] p-2 rounded-md text-white mr-3 w-11 h-11 flex text-2xl justify-center align-middle">
            ...
            <Link href={facebookLink} target="_blank" className="bg-[#3f729b] p-2 rounded-md text-white w-11 h-11 flex text-2xl justify-center align-middle">
